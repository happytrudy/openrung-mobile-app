...
consumed as the Go module `github.com/happytrudy/openrung/punchcore` at the
...
as the tagged Go module `github.com/happytrudy/openrung/wsscore`, currently pinned
to **v0.7.0**
...
tagged Go module `github.com/happytrudy/openrung/brokerapi`, currently pinned to
...
3. Dependabot ([`.github/dependabot.yml`](.github/dependabot.yml), scoped to
   ...
   `go get github.com/happytrudy/openrung/punchcore@vX.Y.Z` inside
...
```sh
GOWORK=off go get github.com/happytrudy/openrung/wsscore@vX.Y.Z
```
...
```sh
GOWORK=off go get github.com/happytrudy/openrung/brokerapi@vX.Y.Z
```
...
`github.com/happytrudy/openrung/connectcore`. Use only a released
...
```sh
GOWORK=off go get github.com/happytrudy/openrung/connectcore@vX.Y.Z
```
...
`github.com/happytrudy/openrung/brokerapi` version (the
`brokerapi/vX.Y.Z` tag) +
the pinned
`github.com/happytrudy/openrung/connectcore` version (the `connectcore/vX.Y.Z`
tag) +
the pinned
`github.com/happytrudy/openrung/punchcore` version (the `punchcore/vX.Y.Z` tag in
the `openrung/openrung` repository recorded in `android/punchbridge/go.mod`) +
the pinned `github.com/happytrudy/openrung/wsscore` version (`wsscore/vX.Y.Z`)
